...
- Simulate data from <br/> $x_1 \sim N(1,1)$ and $x_2\sim N(10,10)$ <br/>
...
dist1 <- function(N){rnorm(N, 10, 5 )}
dist2 <- function(N){rnorm(N, 20, 10)}
...
- Simulate data from <br/> $x_1 \sim N(1,1)$ and $x_2\sim N(10,10)$ <br/> (20 each)
- compute p-values using:
  - randomization test
  - t-test
  - asymptotic test <br/> (T under null $\sim N(\mu, \sigma)$)
...
dist2 <- function(N){rnorm(N, 10, 10)}
xx <- p3003PBC::sample_stats(N_obs = 40,
                   dist1,
                   dist2,
                   samples = 100)
cbind( coin = table(xx$res_p[,"coin"] < 0.01),
...
with(sleep, t.test(extra[group == 1 ], extra[group == 2] ))$p.value
