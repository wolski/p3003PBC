...
background-image: url("images/test_chair.jpeg")
...
unpaired.p =
  t.test(extra ~ group, #<<
         data = sleep)$p.value, #<<
paired.p =
  t.test(sleepwide$`1`, sleepwide$`2`, #<<
         paired = TRUE #<<
         )$p.value, #<<
diff.p =
...
# This fails - t.test doesn't support this formula
tryCatch(
  t.test(extra ~ group + ID, data = sleepless),#<<
  error = function(e) e)

# Unpaired t-test works but ignores pairing
tryCatch(
  t.test(extra ~ group, data = sleepless),#<<
  error = function(e) e)

# Correct approach: reshape and use paired argument
sleepless_wide <- tidyr::spread(sleepless, group, extra)
t.test(sleepless_wide$`1`, sleepless_wide$`2`, #<<
       paired = TRUE) #<<

```

.footnote[The formula `extra ~ group + ID` fails with t.test. To handle paired data with missing values, reshape to wide format and use `paired = TRUE`.]
