...
- Many tests rely on __assymptotic properties__
...
# Repeated - correlated measurments
...
xx <- add_column(x, model = c("lm_1","lm_2","lmer"), .before = 1) %>%
  select(model, p.value) %>% mutate(p.value = signif(p.value, digits=2))
...
- What is the null distribution of the test stastic
- There might be no non-parametric alternative.
- Asymptotic properties are used (CLT) to derive H0 for complex models.
