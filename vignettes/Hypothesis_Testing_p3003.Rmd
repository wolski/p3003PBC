...
The variance of the sampling distribution of the mean is the population variance divided by $n$ ($iid$ assumption).
...
- Many tests rely on __asymptotic properties__
...
sleep %>% tidyr::spread(group, extra) -> sleepwide
colnames(sleepwide) <- make.names(colnames(sleepwide))
plot(sleepwide$X1, sleepwide$X2) #<<
legend("topleft", legend = paste("cor = ",round(cor(sleepwide$X1, sleepwide$X2), digits=2)))
sleepwide <- sleepwide %>% 
...
xx <- add_column(x, model = c("lm_1","lm_2","lmer"), .before = 1) %>%
  dplyr::select(model, p.value) %>% mutate(p.value = signif(p.value, digits=2))
...
lm1 <- lm(extra ~ group,
          data = sleepless) #<<
lm2 <- lm(extra ~ group + ID, data = sleepless)
lmermod <- lmerTest::lmer(extra ~ group + (1|ID), data = sleepless)
...
- What is a test statistic and the null distribution
- There are parametric tests and there might be no non-parametric alternative.
- Parametric tests make assumptions about the data.
- Asymptotic properties are used (CLT) to derive null distributions for complex models.
- In practice, if you are not sure about meeting assumptions, rely on effect sizes, confidence intervals and back them up with degrees of freedom.
