...
- Simulate data from <br/> $x_1 \sim N(1,1)$ and $x_2\sim N(10,10)$ <br/> (5 each)
...
xx <- add_column(x, 
        ...
        .before = 1) %>%
  dplyr::select(model, p.value) %>%
  dplyr::mutate(p.value = signif(p.value, digits = 2))
```
]
...
state if you reject null given your size of test $\alpha$ <br/>)
