...
boot <- function(x){ x <- sample(my_sample , N_obs,replace = TRUE); return(data.frame(mean = mean(x), sd= sd(x)))}
res <- purrr::map_df(1:N, boot)
library(tidyverse)
res <- res %>% filter(sd > 0)
T0 <- res$mean/(res$sd/sqrt(N_obs))

hist(T0, freq = F,breaks=getBreaks(T0))
lines(x,dt(x,df = N_obs),type="l",col="green",lwd=2)
