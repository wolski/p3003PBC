...
.footnote[Data vs. Hypothesis driven research. Data driven use exploratory and descriptive tools. Hypothesis driven use tests.] 
...
.image-50[![](../inst/images/JellyBeans_significant.png)]
...
- __Repeated measures__
  ...
  - Solution: 
      - two-sample t-tests at each time point in sequence,  adjust p.value
...
Family-wise error rate is the probability of making one or more false discoveries, or Type I errors when performing multiple 
hypotheses tests.
...
Multiplicity adjustment, controlling the FWER, will demand a *unrealistically
small p-value*. 

In practice there are better ways of overcoming the problem
of multiplicity: 
- limit the number of tests. Concentrating on the more important objectives, what is causal relation.
...
- If you want to examine many factors, use exploratory or descriptive data analysis (tabulating, dimensionality reduction, clustering).

---

# FDR Motivation

.footnote[Simulating p-values Figure A) 1000 p-values where H0 true, B) 900 p-values where H0 true and 100 HA true, C) 800 p-values where H0 true and 200 HA true
...
abline(v = 0.05, col=2)
text(x = c(0.02,0.02,0.1,0.1),y=c(30,100,30,100), labels=c("FP","TP","TN","FN"))
