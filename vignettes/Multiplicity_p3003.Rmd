...
- __Repeated measures__
  ...
  - Solution: 
      - two-sample t-tests at each time point in sequence, then  adjust p.value
...
- R function `p.adjust` transforms the p-values instead transforming the threshold.
- see also R package `multcomp`
...
- In Figure D we have p-values less than significance threshold where H0 is true (FP) and a proportion of those where HA is true (TP).
