...
background-image: url("images/Linear_chair.jpg")
...
.footnote[https://www.youtube.com/watch?v=mt9WKbQJrI4]
---
...
.right-plot[
![](images/CausalNetworkSignalTransduction.png)
]
...
- __anchoring__ heuristic (first impression)
  - insufficient adjustment
  - assessment of uncertainty - typically under-estimated

.footnote[
Anchoring can limit consideration of factors or scenarios that lie outside the initial frame of reference, reducing the accuracy of uncertainty assessments. 
For instance, a new gene's function might be inferred based on a reference gene's function, even if adjustments for species-specific differences are insufficient.]
...
- to prior probability of outcome $^{\star\star}$
- to sample size
- to predictability (predict future based on observations made now), is connected to anchoring $^{\star}$
- illusion of validity caused by consistency  $^{\star}$
- misconception of regression - confounding $^{\star\star}$


.footnote[
$^{\star\star}$ fail to properly account for the base rate or prior likelihood of an event occurring when making judgments or decisions

$^{\star}$ Overestimate or underestimate how well certain factors or cues can predict future outcomes or explain phenomena
...
# Mitigating Insensitivity to Predictability

- **Probabilistic Thinking**:  
  Encouraging the use of probabilities and statistical tools helps people better assess the predictability of outcomes.

- **Training and Feedback**:  
  Repeated exposure to prediction tasks with feedback can improve sensitivity to the true predictability of events.

- **Avoiding Anchoring**:  
  Ensuring people critically evaluate all relevant information rather than fixating on initial values or assumptions helps reduce biases in predicting outcomes.
...
.footnote[https://www.youtube.com/watch?v=HZGCoVF3YvM]
...
3. __Specificity__ – evaluates whether a specific cause leads to a specific effect
4. __Temporality__ – effect follows cause, and if expected delay, effect must occur after that delay (only mandatory criterion)
...
![](images/BH_Plausibility.png)]

.pull-right[Specificity<br/>
![](images/BH_Specificity.png)]
...
.img-right[
![](images2021/Bergmann_Murdoch.png)
]
...
.image-70[
![](images/Study_Type_Case_Control.png)
]
...
.img-right[![](images/blind_trials.png)]
...
.img-small[
![](images/Causality_Mechanisms_3.png)
]
...
# Confounders - Types of

.image-70[
![](images_2/FourElementalConfounds.png)
]

.footnote[https://www.youtube.com/watch?v=mBEA7PKDmiY]
...
.img-right[
![How to avoid](images/BatchEffect.png)
...
- No interaction
.img-right[
![](images/TypesOfInteractions_NO.png)
...
.img-right[
![](images/TypesOfInteractions_Quanti.png)
]
...
.img-right[
![](images/TypesOfInteractions_Quali.png)
]
...
.img-right[
![](images/VariousTypesOfVariation.jpg)
![](images/VariousTypesOfVariation_Summary.jpg)
]
...
.footnote[https://www.youtube.com/watch?v=uanZZLlzKHw]
